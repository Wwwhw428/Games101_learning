...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    float min_x = std::min(t.v[0][0], std::min(t.v[1][0], t.v[2][0]));
    float max_x = std::max(t.v[0][0], std::max(t.v[1][0], t.v[2][0]));
    float min_y = std::min(t.v[0][1], std::min(t.v[1][1], t.v[2][1]));
    float max_y = std::max(t.v[0][1], std::max(t.v[1][1], t.v[2][1]));
    ...
    for (int x = min_x; x <= max_x; x++)
    {
        for (int y = min_y; y <= max_y; y++)
        {
            ...
            {
                auto baryCenter = computeBarycentric2D(x, y, t.v);
                float alpha, beta, gamma;
                std::tie(alpha, beta, gamma) = baryCenter;
